...
def parse_data_js(file_path):
    ...
    for suite_name, suite_data in data.get("entries", {}).items():
        platform = "unknown"
        if "ubuntu" in suite_name.lower(): platform = "ubuntu-latest"
        elif "windows" in suite_name.lower(): platform = "windows-latest"
        elif "pyodide" in suite_name.lower(): platform = "Pyodide"
...
def normalize_msg(msg):
    ...
    msg = msg.lower()
    msg = re.sub(r'\(#\d+\)', '', msg)
    msg = re.sub(r'^(feat|fix|chore|docs|style|refactor|test|perf|ci|release):\s*', '', msg)
...
def recover_data(original_data_path, output_dir):
    ...
    keywords = ["streaming", "wasm", "echarts", "bytecode", "normalization", "validation", "numpy", "pandas", "polars"]
    
    for commit in main_commits:
        if commit["sha"] in mapped_shas: continue
        n_main = normalize_msg(commit["msg"])
    ...
    print(f"Recovery complete: {count} entries atomized. {len(mapped_shas)} unique SHAs mapped.")
