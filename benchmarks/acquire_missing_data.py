...
def run_command(cmd, cwd=None):
    ...
    try:
        return subprocess.check_output(
            cmd, shell=True, text=True, stderr=subprocess.STDOUT, cwd=cwd
        )
...
def acquire_data(target_shas_path, output_dir, limit=5):
    ...
    for sha in shas:
        ...
        # Standard Benchmarks
        res_standard = run_command(
            "POLARS_SKIP_CPU_CHECK=1 PYTHONPATH=src:. venv/bin/python3 benchmarks/run.py --json"
        )
        if res_standard:
            try:
        ...
        if Path("benchmarks/streaming.py").exists():
            ...
            res_streaming = run_command(
                "POLARS_SKIP_CPU_CHECK=1 PYTHONPATH=src:. venv/bin/python3 benchmarks/streaming.py --json"
            )
            if res_streaming:
                try:
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python3 acquire_missing_data.py <missing_shas.txt> <output_dir> [limit]"
        )
        sys.exit(1)
