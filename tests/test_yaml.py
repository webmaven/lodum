...
def test_yaml_roundtrip_simple():
    """Tests a simple object can be serialized and deserialized."""
    instance = Simple(a=10, b="world")
    yaml_str = to_yaml(instance)
    result = from_yaml(Simple, yaml_str)
...
def test_yaml_datetime():
    ...
    yaml_str = to_yaml(dt)
    # ruamel.yaml may add quotes, so we deserialize to check correctness.
    result = from_yaml(datetime, yaml_str)
    assert result == dt

def test_yaml_enum():
    """Tests Enum objects are handled correctly."""
    role = UserRole.ADMIN
    yaml_str = to_yaml(role)
    # ruamel.yaml may add a document end marker, so we check the content.
    assert "admin" in yaml_str
    result = from_yaml(UserRole, yaml_str)
    assert result == role
...
def test_yaml_roundtrip_complex():
    """Tests a complex nested object can be serialized and deserialized."""
    instance = Complex(
        dt=datetime.now(),
        role=UserRole.USER,
    ...
    yaml_str = to_yaml(instance)
    result = from_yaml(Complex, yaml_str)
    assert result == instance
...
def test_yaml_field_customization():
    ...
    # Test serialization with `rename` and `skip_serializing`
    yaml_str = to_yaml(instance)
    assert "id: 123" in yaml_str
    assert "user_id" not in yaml_str
    assert "is_active" not in yaml_str

    # Test deserialization with `rename` and `default_factory`
    yaml_data = "id: 456\n"
    result = from_yaml(Customized, yaml_data)
...
class TypingObject:
    ...
    def __eq__(self, o):
        # The YAML library deserializes lists/dicts as a custom type, so we need to
...
def test_yaml_typing_support():
    ...
    yaml_str = to_yaml(instance)
    result = from_yaml(TypingObject, yaml_str)
